...
import {ArrivalInfo, CityApi, LocationInfo} from "../cities/CityTypes.js";
import {Config} from "./Config.js";
import {MetroNetwork, Train} from "./Types.js";
...
export class NetworkHandler {
    ...
    private readonly trainLayer = new paper.Layer();
     ...
     */
    constructor(metroNetwork: MetroNetwork, cityApi: CityApi) {
    ...
    private async fetchTrainData() {
        ...
        for (const arrival of arrivals) {
            if (!this.validateArrival(arrival)) continue;
        ...
        if (newTrainArrivals) await this.handleNewTrainArrivals(newTrainArrivals);
    }
     ...
     * @private
     */
    private validateArrival(arrival: ArrivalInfo): boolean {
        const metroLine = this.metroNetwork.getMetroLine(arrival.line);
        if (!metroLine) {
     ...
     * @private
     */
    private handleExistingArrival(train: Train, arrival: ArrivalInfo) {
        ...
        if (location.isStation) {
            ...
            if (location.station.id === arrival.stationId) return;
            ...
            if (location.arrivalStation.id === arrival.stationId) return;
     ...
     * @private
     */
    private async handleNewTrainArrivals(arrivals: ArrivalInfo[]) {
     ...
     * @private
     */
    private handleNewTrainArrival(locationInfo: LocationInfo) {
        const arrival = locationInfo.nextArrival;
        const location = locationInfo.location;
        if (!this.validateArrival(arrival)) return;
        ...
        if (Array.isArray(location)) {
            ...
            const train = TrainImpl.newTrainOnEdge(arrival.trainId, metroLine, edge, arrival.arrivalTime,
                proportion * edge.length, this.trainLayer, this.metroNetwork.lineWidth, metroLine.color);
            ...
            const train = TrainImpl.newTrainAtStation(arrival.trainId, metroLine, currentStation,
                [arrivalStation, arrival.arrivalTime], this.trainLayer, this.metroNetwork.lineWidth, metroLine.color);
            metroLine.addTrain(train);
        }
    }
